...
function HomePage() {
	...
	const { impactMetrics, blogPosts, partners } = useDataStore();
		...
		<div className="space-y-16">
			...
			<section>
				...
				<div className="grid md:grid-cols-3 gap-6">
					{impactMetrics.slice(0, 3).map((metric, idx) => (
						<Card key={idx}>
			...
			<section>
				<div className="text-center mb-8">
					...
					<p className="text-muted-foreground">Updates, stories, and transparency reports</p>
				</div>
				<div className="grid md:grid-cols-3 gap-6">
						...
						<Card key={post.id}>
							...
							<CardContent>
								<div className="flex items-center gap-2 text-xs text-muted-foreground mb-4">
									<span>{new Date(post.publishedDate).toLocaleDateString("en-GB", { month: "short", day: "numeric" })}</span>
									<span>&bull;</span>
			...
			{/* About GiveTransparent */}
			<section className="bg-muted/30 -mx-4 px-4 py-12 rounded-lg">
				<div className="max-w-3xl mx-auto text-center space-y-6">
					<h2 className="text-3xl font-bold">About GiveTransparent</h2>
					...
					<div className="grid md:grid-cols-3 gap-6 pt-6">
						<div className="text-center">
							...
							<div className="text-sm font-medium">Goes to Partners</div>
							<div className="text-xs text-muted-foreground mt-1">Every quarter</div>
						</div>
						<div className="text-center">
							<div className="text-4xl font-bold text-primary mb-2">100%</div>
							<div className="text-sm font-medium">Fee Transparency</div>
							<div className="text-xs text-muted-foreground mt-1">All fees disclosed</div>
						</div>
						<div className="text-center">
							<div className="text-4xl font-bold text-primary mb-2">{partners.filter(p => p.verified).length}</div>
							<div className="text-sm font-medium">Verified Partners</div>
							<div className="text-xs text-muted-foreground mt-1">Rigorously vetted</div>
						</div>
					</div>
					<Link to="/about">
